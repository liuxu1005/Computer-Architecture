...
\section*{}
For median and large size data set, sequential sorting outperforms Parallel sorting as shown in Figure 2 and 3. This result was not expected by us. This is the first time we program on GPU and also first time use cuda.  We think the reason why the sequential sorting are so fast is that CPU has been optimized well enough by pipeline, branch prediction, cache policy and so on. Well why the parallel sorting are slow? We continue to profile all the parallel to see details.
...
Figure 5 tells us that the last few rounds of parallel merge sort only use few threads and therefore consume most of the time. We think the 
...
In Figure 6, we only cite very small part of the profiling since there are many same module which number is up to the same as the data size.
...
\label{fig-knearest-perf}
\end{figure}
Figure 7 shows the bitonic can exploit as many threads as half of the data size, which make it fast. We think it is the only parallel algorithm
...
\end{figure}
\section*{}
Figure 8, 9 and 10 show the Even-Odd sorting algorithm are much still as profiling shows it takes to many rounds to guarantee the data sorted.
...
\section{Conclusion}
 ...
 Although Parallelism is a promising direction to take the advantage of  Moore's law, the programming model is an significant factor affect whether the multiple threads can be used well. Our project most of sorting algorithm don't fit well the GPU programming model. \\
...
\begin{small}
\bibliographystyle{abbrv} \bibliography{[1] Daniel Cederman and Philippas Tsigas. (2010) GPU-Quicksort: A practical Quicksort algorithm for graphics processors. J. Exp. Algorithmics 14, Article 4 (January 2010), .84 pages.
}
\bibliographystyle{abbrv} \bibliography{[2] Ha L, Kruger J, Silva TC (2009) Fast four-way parallel radix sorting on GPUs. Comput Graph Forum 28(8):2368–2378}
\bibliographystyle{abbrv} \bibliography{}
\bibliographystyle{abbrv} \bibliography{}
\end{small}
